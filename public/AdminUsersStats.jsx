...
function UsersPage({ toast }) {
  ...
  React.useEffect(() => { window.AICDB_AUTH.getClient().then(setSb); }, []);

  React.useEffect(() => {
    if (!sb) return;
    setLoading(true);
    sb.rpc('get_users_for_admin').then(({ data, error }) => {
      if (error) {
        // Fallback: query profiles directly (no email column access from anon)
        sb.from('profiles').select('id, username, display_name, is_admin, created_at').order('created_at', { ascending: false })
          .then(({ data: profiles }) => {
            const mapped = (profiles || []).map(p => ({
              id: p.id, name: p.display_name || p.username || '—',
              handle: '@' + (p.username || p.id.slice(0, 8)),
              email: '—', joined: (p.created_at || '').slice(0, 10),
              role: p.is_admin ? 'admin' : 'viewer', works: 0, banned: false,
              av: ['#4ecdc4', '#6f9ceb'],
            }));
            setUsers(mapped);
            window.ADMIN_STATS.totalUsers = mapped.length;
            setLoading(false);
          });
        return;
      }
      const mapped = (data || []).map(u => ({
        id: u.id, name: u.display_name || u.username || '—',
        handle: '@' + (u.username || u.id.slice(0, 8)),
        email: u.email || '—', joined: (u.created_at || '').slice(0, 10),
        role: u.is_admin ? 'admin' : 'viewer', works: 0, banned: false,
        av: ['#4ecdc4', '#6f9ceb'],
      }));
      setUsers(mapped);
      window.ADMIN_STATS.totalUsers = mapped.length;
  ...
  const onAction = (mode, u) => {
    ...
    if (mode === 'unban') { setUsers(list => list.map(x => x.id === u.id ? { ...x, banned:false } : x)); toast(u.name + ' reinstated'); }
    if (mode === 'warn')  { toast('Warning sent to ' + u.name); }
    if (mode === 'view')  { toast("Opening " + u.handle + "'s profile..."); }
    if (mode === 'toggleCreator') {
  ...
  return (
    <div>
      <APageHead title="Users" sub={users.length + ' registered accounts'}>
