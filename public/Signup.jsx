...
function StepChoice({ onEmail }) {
  ...
  const socials = [
    { key:'google', icon:<GoogleIcon size={18} />, label:'Sign up with Google',
      onClick: () => { setError(''); window.AICDB_SIGNUP_GOOGLE && window.AICDB_SIGNUP_GOOGLE(setError); } },
  ];
  return (
    <div>
      <Header title="Create your account" sub="Join Dreamwall to rate, review and track AI-generated films & series." />
      {error && <p style={{ font:'var(--text-body-sm)', color:'var(--danger)', textAlign:'center', margin:'0 0 10px' }}>{error}</p>}
...
function StepForm({ onBack, onSubmit }) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      if (window.AICDB_SIGNUP_EMAIL) {
        await window.AICDB_SIGNUP_EMAIL(
      ...
      // AICDB_SIGNUP_EMAIL redirects on success; reaching here means no handler � advance step.
      onSubmit(f.email.trim().toLowerCase());
    } catch (err) {
    ...
    <div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        <div style={{ display:'flex', gap:12 }}>
          <div style={{ flex:1, minWidth:0 }}><Field label="First name" value={f.first} error={errors.first} onChange={set('first')} placeholder="Ada" onKeyDown={e=>{ if(e.key==='Enter') submit(); }} /></div>
          <div style={{ flex:1, minWidth:0 }}><Field label="Last name" value={f.last} error={errors.last} onChange={set('last')} placeholder="Lovelace" onKeyDown={e=>{ if(e.key==='Enter') submit(); }} /></div>
        </div>
        ...
        <div>
          <Field label="Password" type={showPw?'text':'password'} value={f.pw} error={errors.pw} onChange={set('pw')} placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;"
            onKeyDown={e=>{ if(e.key==='Enter') submit(); }}
            trailing={
        ...
        <div>
          <Field label="Confirm password" type={showPw?'text':'password'} value={f.pw2} error={errors.pw2} onChange={set('pw2')} placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;" onKeyDown={e=>{ if(e.key==='Enter') submit(); }} />
        ...
        <PrimaryWideButton onClick={submit} disabled={submitting}>
...
function Signup() {
  ...
  return (
    <CardShell>
      ...
      {step === 2 && <StepForm onBack={()=>setStep(1)} onSubmit={()=>{}} />}
    </CardShell>
  );
}
