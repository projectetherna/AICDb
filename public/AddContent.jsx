        ...
        <>
          ...
          {series ? (
            <div style={{ display:'flex', gap:16 }}>
              ...
              <div style={{ flex:1 }}><Field label="Episode length"><DurationPicker value={form.duration} onChange={v => set({ duration:v })} /></Field></div>
            </div>
          ) : (
            <Field label="Duration" hint="Runtime in plain form, e.g. 45m or 1h30m."><DurationPicker value={form.duration} onChange={v => set({ duration:v })} /></Field>
          )}
...
function Step2({ form, set, adminMode }) {
  const series = form.type === 'series';
  return (
    <div>
      <SectionTitle icon="image" hint="The hero photo runs behind the title-page header; the motto sits over it.">Hero</SectionTitle>
      {adminMode ? (
        <Field label="Background photo URL" hint="Paste a public image URL (16:9 recommended).">
          <TextInput value={form.heroImage || ''} onChange={v => set({ heroImage: v })} placeholder="https://…" />
        </Field>
      ) : (
      ...
      <div style={{ display:'flex', gap:20 }}>
        <div style={{ width:150, flex:'none' }}>
          {adminMode ? (
            <Field label="Poster URL" hint="2:3 ratio recommended.">
              <TextInput value={form.poster || ''} onChange={v => set({ poster: v })} placeholder="https://…" />
            </Field>
          ) : (
            <Field label="Poster">
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <Field label="Genre / type tags" hint="Press Enter to add. Sci-Fi, Drama, Animation…">
            <GenrePicker tags={form.genres} onChange={v => set({ genres:v })} />
          </Field>
      ...
      <SectionTitle icon="play" hint={series ? 'Paste an embed per episode.' : 'Paste the player embed code or a direct video URL.'}>Playback</SectionTitle>
      {series ? (
        <EpisodeEmbeds rows={form.episodeEmbeds} onChange={v => set({ episodeEmbeds:v })} />
      ) : (
        <Field label="Embed code or video URL">
          <TextArea value={form.embed} onChange={v => set({ embed:v })} rows={3} mono
...
function Step3({ form, set, adminMode }) {
...
function AddContent() {
  ...
  const hasEmbed = form.type === 'series'
    ? form.episodeEmbeds.some(e => e.embed && e.embed.trim())
    : !!(form.embed && form.embed.trim());
  const newCanSubmit = form.agreeRights && form.agreeAI && form.title.trim() && hasEmbed;
  ...
  const submitNew = async () => {
    if (!newCanSubmit) { setTried(true); return; }
    setSaving(true);
    setSaveError(null);
    try {
      ...
      const typeMap = { movie: 'film', series: 'series', short: 'short' };
      const dbType = typeMap[form.type] || form.type;

      const durationMin = (() => {
        const s = (form.duration || '').trim();
      ...
      const credits = (form.crew || []).filter(c => c.name && c.name.trim());

      const row = {
        type: dbType,
        status: 'pending',
        title: form.title.trim(),
        slug: makeSlug(form.title.trim()),
        synopsis: form.description.trim() || null,
        genres: form.genres || [],
        poster_url: typeof form.poster === 'string' ? form.poster : null,
        release_year: form.year ? parseInt(form.year) : null,
        duration_minutes: durationMin,
        embed_code: form.embed.trim() || null,
        ai_tools: form.models.length ? form.models : null,
        credits: credits.length ? credits : null,
        submitted_by: session.user.id,
      };

      const { data: inserted, error } = await sb.from('content').insert(row).select('id').single();
      ...
      if (creatorProfile?.id) {
        await sb.from('content').update({ creator_id: creatorProfile.id }).eq('id', inserted.id);
      }

      setSubmitted(true);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } catch (e) {
      setSaveError(e.message || 'Could not submit. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ minHeight:'100vh' }}>
      <NavBar active="" onNav={(label) => { window.location.href = '/index.html#' + encodeURIComponent(label); }} query={query} onQuery={setQuery} onOpenResult={() => {}} />
              ...
              <div style={{ background:'var(--bg-1)', borderRadius:'var(--radius-xl)', borderWidth:1, borderStyle:'solid', borderColor:'var(--border-subtle)',
                ...
                {tried && ((step === 1 && !form.title.trim()) || (step === 3 && !newCanSubmit)) && (
                  <div style={{ display:'flex', alignItems:'center', gap:9, marginTop:20, padding:'11px 14px', borderRadius:'var(--radius-md)',
                    ...
                    font:'var(--text-body-sm)', color:'var(--score-low)' }}>
                    <Icon name="warning" size={15} color="var(--score-low)" />
                    {step === 1 ? 'A title is required to continue.' : (
                      !form.title.trim() ? 'A title is required.' :
                      !hasEmbed ? 'A video link or embed code is required.' :
                      (!form.agreeRights || !form.agreeAI) ? 'Confirm both declarations to submit.' : ''
                    )}
                  </div>
                )}
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:28, paddingTop:24,
                  ...
                  {step < 3 ? (
                    ...
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:8 }}>
                      <button onClick={submitNew} disabled={saving} style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'11px 20px', borderRadius:'var(--radius-md)',
                        border:'none', cursor: newCanSubmit && !saving ? 'pointer' : 'not-allowed', font:'600 14px/1 var(--font-body)',
                        background: newCanSubmit && !saving ? 'var(--coral)' : 'var(--bg-3)', color: newCanSubmit && !saving ? 'var(--fg-on-accent)' : 'var(--fg-3)',
                        transition:'background var(--dur-fast)' }}>
                        <Icon name="check" size={16} color={newCanSubmit && !saving ? 'var(--fg-on-accent)' : 'var(--fg-3)'} weight="bold" />
                        {saving ? 'Submitting...' : 'Submit title'}
                      </button>
                      {saveError && (
                        <div style={{ font:'var(--text-body-sm)', color:'var(--score-low)' }}>{saveError}</div>
                      )}
                    </div>
                  )}
                </div>
...
Object.assign(window, { AddContent, Stepper, Step1, Step2, Step3 });
if (document.getElementById('root') && !window.__AICDB_ADMIN__) {
