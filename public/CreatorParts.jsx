...
function FollowButton({ creator }) {
  const [following, setFollowing] = React.useState(() => window.AICDB_FOLLOWS?.isFollowing('creator', creator.id) || false);
  const [hover, setHover] = React.useState(false);

  React.useEffect(() => window.AICDB_FOLLOWS?.subscribe(() => setFollowing(window.AICDB_FOLLOWS.isFollowing('creator', creator.id))), [creator.id]);

  const n = (creator.followers ?? 0) + (following ? 1 : 0);
  ...
  return (
    <div style={{ display:'inline-flex', alignItems:'center', gap:16 }}>
      <button onClick={() => {
        ...
        window.AICDB_FOLLOWS.toggle('creator', creator.id);
...
function CreatorHero({ creator, isOwner, manage, onEditClick }) {
  return (
    <div style={{ position:'relative', marginBottom:140 }}>
      ...
      <div style={{ position:'absolute', left:0, right:0, bottom:-128, display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center', padding:'0 24px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:24, margin:'26px 0 0', flexWrap:'wrap', justifyContent:'center' }}>
          {isOwner ? (
            <button onClick={() => onEditClick && onEditClick(true)}
              style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'11px 22px', borderRadius:'var(--radius-pill)', cursor:'pointer',
              ...
              <Icon name="pencil" size={15} color="var(--fg-1)" /> Edit Profile
            </button>
          ) : (
          ...
          <div style={{ display:'flex', gap:10 }}>
            {[
              ['youtube-logo', 'YouTube', creator.social.youtube],
              ['instagram-logo', 'Instagram', creator.social.instagram],
              ['x-logo', 'X', creator.social.x],
              ['tiktok-logo', 'TikTok', creator.social.tiktok],
              ['globe', 'Website', creator.social.website],
            ].filter(([, , href]) => href && href !== '#').map(([icon, label, href]) => (
...
function CreatorStatStrip({ creator, totalViews }) {
  const worksCount = creator.worksCount ?? 0;
  const totalRatings = creator.totalRatings ?? 0;
  const avgScore = creator.avgScore;
  const items = [
    { label:'Works',        value: worksCount, icon:'film-slate',      color:'var(--coral-bright)' },
    { label:'Total Views',  value: totalViews ?? '—', icon:'eye',     color:'var(--fg-1)' },
    { label:'Avg Score', value: avgScore != null ? avgScore.toFixed(1) : '—', unit: avgScore != null ? '/10' : '', icon:'sparkle', color:'var(--teal-bright)', scoreColor: avgScore != null ? scoreColor(avgScore) : 'var(--fg-3)' },
    { label:'Ratings',      value: fmtCount(totalRatings), icon:'star',   color:'var(--coral-bright)' },
    { label:'Followers',    value: fmtCount(creator.followers ?? 0), icon:'users-three', color:'var(--fg-1)' },
  ];
  return (
